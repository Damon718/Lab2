...
//获取图片点阵信息
function getImgTemplateDots() {
    var img = imageTemplateManager.randomSource();
    if (!img) return null;

    var scale = Math.random() * 0.5 + 0.6;
    var density = 2;
    var dots = [];
...
function createImgBurst(particleFactory, center_x, center_y) {
    var map = getImgTemplateDots();
    if (!map) return;
    var dcenterX = map.width / 2;
    var dcenterY = map.height / 2;
    var color = randomColor();
    ...
    burst(x, y) {
        ...
        if (!this.disableWordd && store.state.config.wordShell) {
            ...
            //     if (Math.random() < 0.5) {
                    switch (i) {
                        case 0:
                            createWordBurst(randomWords[0], dotStarFactory, 700, 600); i++;
                            break;
                        case 1:
                            createWordBurst(randomWords[1], dotStarFactory, 700, 700); i++;
                            break;
                        case 2:
                            createWordBurst(randomWords[2], dotStarFactory, 700, 800);i++;
                            break;
                        case 3:
                            createWordBurst(randomWords[3], dotStarFactory, 700, 900); i++;
                            break;
                        case 4:
                            createWordBurst(randomWords[4], dotStarFactory, 700, 1000); i++;
                            break;
                        case 5:
                            createWordBurst(randomWords[5], dotStarFactory, 700, 1100); i++;
                            break;
                        case 6:
                            createWordBurst(randomWords[6], dotStarFactory, 700, 1200); i=0;
                            break;
                    }
            //     } else {
